...
def search_wikihow(keyword,event_name):
    ...
    # print(search_link[:20])

    links = []

    for link in search_link[:20]:
        if 'Category' not in link:
            ...
            cat_tags = new_soup.find(id="cat_all").find(class_="cat_grid")
            cnt = 0
...
if __name__ == '__main__':
    keyword_list = ['Propose+Marriage','Attempting+a+bike+trick','Cleaning+an+appliance']
    keyword = 'changing+a+vehicle+tire'
    search_wikihow(keyword,event_name='Working_on_a_sewing_project')
    # test()
